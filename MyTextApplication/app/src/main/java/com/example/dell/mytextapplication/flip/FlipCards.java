...
import com.example.dell.mytextapplication.fliputils.TextureUtils;
import com.example.dell.mytextapplication.fliputils.UI;
  ...
  public synchronized boolean handleTouchEvent(MotionEvent event, boolean isOnTouchEvent) {
    switch (event.getAction()) {
      case MotionEvent.ACTION_DOWN:
        // remember page we started on...记住开始的页面
        lastPageIndex = getPageIndexFromAngle(accumulatedAngle);
        lastPosition = orientationVertical ? event.getY() : event.getX(); //如果是竖屏翻页，记住y坐标，如果是横屏翻页，记住x坐标
        return isOnTouchEvent;
      case MotionEvent.ACTION_MOVE:
        float delta = //用来判断移动的方向，如果大于0，则正方向移动，如果小于0，则反方向移动
        ...
        if (state == STATE_TOUCH) {
          if (Math.abs(delta) > MIN_MOVEMENT) //ignore small movements 假如移动范围很小，则忽略不计
          {
            forward = delta > 0;
          }

          controller.showFlipAnimation();  //开始滑动，同时opengl es进行渲染

          float angleDelta; //将滑动的距离，换算成页面翻转的角度
          ...
          {//返回参数 a 的符号函数;如果参数是零,则返回零; 如果参数大于零,则返回 1.0;如果参数小于零,则返回-1.0
            angleDelta = Math.signum(angleDelta) * MAX_TOUCH_MOVE_ANGLE;
          }

          // do not flip more than one page with one touch...  一次滑动，只能滑动一页
          if (Math.abs(getPageIndexFromAngle(accumulatedAngle + angleDelta) - lastPageIndex) <= 1) {
            accumulatedAngle += angleDelta;
          }

          //Bounce the page for the first and the last page，防止页数越界
